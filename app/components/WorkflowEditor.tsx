...
export function WorkflowEditor({ workflowDefinition }: WorkflowEditorProps) {
  ...
  const [userStateFormMappings, setUserStateFormMappings] = useState<
  ...
  // Compute state form mappings directly from the queries
  const stateFormMappings = useMemo(() => {
    const computedStateFormMappings: Record<string, number> = {};
    for (const { state, formDef } of formQueries) {
      if (formDef?.formDefId) {
        computedStateFormMappings[state] = formDef.formDefId;
      }
    }
    return {
      ...computedStateFormMappings,
      ...userStateFormMappings,
    };
  }, [formQueries, userStateFormMappings]);
  ...
  const handleSave = useCallback(async () => {
    try {
      ...
      const machineConfig = {
        ...
        states: states.reduce((acc, state, index) => {
          const onConfig: Record<string, string> = {};
          if (index < states.length - 1) {
            const nextState = states[index + 1];
            if (nextState) {
              onConfig.NEXT = nextState;
            }
          }
          if (index > 0) {
            const prevState = states[index - 1];
            if (prevState) {
              onConfig.PREVIOUS = prevState;
            }
          }
          acc[state] = { on: onConfig };
          return acc;
        }, {} as Record<string, StateConfig>),
      };

      // Convert stateFormMappings to array format
      const mappings = Object.entries(stateFormMappings).map(
        ([state, formDefId]) => ({
          state,
          formDefId,
        })
  ...
  }, [states, stateFormMappings, workflowDefinition.id]);
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Select
                  value={stateFormMappings[selectedState]?.toString()}
                  onValueChange={(value) =>
                    setUserStateFormMappings({
                      ...userStateFormMappings,
