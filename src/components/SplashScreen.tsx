...
export const SplashScreen: React.FC<SplashScreenProps> = ({ show, onDismiss }) => {
  return (
    <div
      className={`fixed inset-0 z-50 flex flex-col items-center justify-center bg-black transition-opacity duration-700 ${show ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
    ...
    >
      <div className="flex flex-col items-center w-full max-w-lg px-4 mt-[-10vh]">
        ...
        {/* Title Animation */}
        <h1 className="splash-title-anim text-white text-3xl md:text-5xl font-medium tracking-tight text-center whitespace-nowrap mb-6">
          Welcome to ApproveOS
        </h1>
        {/* Subtitle Animation */}
        <p className="splash-subtitle-anim text-zinc-300 text-lg text-center max-w-md mb-6">
